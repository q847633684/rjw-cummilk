...
本 mod 与其它 mod/系统的集成入口一览，便于查找与开关。RJW、PipeSystem 已收口到 `Source/Integrations/` 目录，命名空间未改。
...
RJW / PipeSystem 各自持有独立 Harmony 实例，在各自 `[StaticConstructorOnStartup]` 中执行，不经过 `EqualMilking`。
...
目录收口已完成：RJW、PipeSystem 已移入 `Source/Integrations/`，命名空间未改。`MilkCum.csproj` 中 Compile 已改为 `..\Integrations\RJW\**\*.cs`、`..\Integrations\PipeSystem\**\*.cs`。
