...
flowchart TB
    ...
    subgraph 进水["进水阶段"]
        ...
        E --> F[按对分组 PairIndex]
        F --> G[每对左/右流速 = …×PressureFactor(P)×R 倍率]
...
- **流速公式**：每对左/右流速 = 该对 hediff 的 RJW × **drive** × 饥饿 × Conditions × Genes × 流速倍率 × **PressureFactor(P)** × **R 倍率**。drive 默认取 L；设置开启激素饱和时取 D_eff = L·H(L)。P = Fullness/maxFullness。**喷乳反射 R 倍率**：吸奶/挤奶后进水量 1.5～2.5 倍（可设 **letdownReflexBoostMultiplier**）维持一段时间，R 衰减后逐渐回 1 倍。公式与参数见第十二节。
...
- **事件触发**  
  挤奶/吸奶：**R ← R + ΔR**，Clamp 至 1。建议 ΔR = 0.3～0.6。
...
- **总流速**  
  **Flow(t) = D_eff × PressureFactor(P) × H_hunger × G × C × [R 倍率]**。  
  - **R 倍率**：加成模式（boost&gt;1）时为 **1 + R×(boost−1)**；旧模式时为 **R**（含下限）。  
