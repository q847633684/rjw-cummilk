...
| 参数 | 影响逻辑 | 建议联动 |
|------|----------|----------|
| rjwBreastSizeEnabled | 总开关：泌乳期临时增大 RJW 乳房 Severity（+0.15）；且是否用 RJW 乳房数据算池容量与流速 | 与 GetBreastCapacityFactors、GetBreastPoolEntries、RJWLactatingBreastSize |
...
以上参数均可通过 Mod 设置保存；关键默认值由 `EqualMilkingDefaultsDef.GetBuiltinDefaults()` 在代码中提供（无 XML），其他 mod 可 patch 该方法修改默认值。
