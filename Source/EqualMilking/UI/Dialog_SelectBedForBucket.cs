...
public class Dialog_SelectBedForBucket : Window
{
    ...
    private void DrawBedRow(float width, ref float row, Building_Bed bed)
    {
        ...
        var owner = bed.GetBedOwner();
        if (owner != null)
            label += " (" + owner.LabelShort + ")";
        if (Widgets.ButtonText(rect, label))
        {
            if (_comp != null)
            {
                if (!_comp.CanLinkToBed(bed))
                {
                    Messages.Message("EM_CumBucket_CannotLinkNotOwner".Translate(), _comp.parent as Thing, MessageTypeDefOf.RejectInput);
                    return;
                }
                _comp.LinkedBed = bed;
            }
