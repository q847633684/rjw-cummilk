...
namespace Cumpilation.Gathering
{
    public class GatheringUtility
    {
        ...
        public static void FillFluidGatherers(SexProps props)
        {
            ...
            {
                var link = gatherer.TryGetComp<CompCumBucketLink>();
                if (link != null && !link.CanPawnUse(pawn))
                    continue;
            ...
            foreach (Hediff genital in relevantGenitals)
            {
                ...
                if (ext.supportedFluids.Contains(fluid))
                {
                    ...
                    if (fgDef != null) {
                        ...
                        gatherer.TryGetComp<CompCumBucketLink>()?.NotifyPawnUsed(props.pawn);
                    } 
                    // Case 2: There is no FluidGatheringDef, but there is a Consumable Def
                    else if (fluid.consumable != null)
                    {
                        // Relevant RJW: consumableFluidRatio = number of consumables per unit fluid; rate 1 -> 10 fluid = 10 ingestions, 0.5 -> 10 fluid = 5.
        ...
        public static FluidGatheringDef LookupGatheringDef(ThingDef filth)
        {
            return DefDatabase<FluidGatheringDef>.AllDefs
                ...
                .Where(def => (def.fluidDef != null && def.fluidDef.filth == filth) || def.filth == filth)
                .FirstOrFallback();
        }
        ...
        public static Building FindClosestFluidSink(Pawn pawn, SexFluidDef fluid)
        {
            ...
            var linkFilter = potentialSinks.Where(x =>
            {
                var link = x.TryGetComp<CompCumBucketLink>();
                return link == null || link.CanPawnUse(pawn);
            }).ToList();

            ModLog.Debug($"Found {linkFilter.Count} reachable Fluid-Sinks for {pawn}s {fluid}");

            return linkFilter.OrderBy(f => f.Position.DistanceTo(pawn.Position)).FirstOrFallback();
        }
