...
public static class EqualMilkLactationApi
{
    ...
    static bool TryGetPoolCompIfRjw(Pawn pawn, out CompEquallyMilkable comp)
    {
        comp = null;
        if (!ModIntegrationGates.RjwModActive || pawn == null) return false;
    ...
    {
        if (!TryGetPoolCompIfRjw(pawn, out var comp)) return false;
    ...
    {
        if (amountPoolUnits <= 0f || !TryGetPoolCompIfRjw(pawn, out var comp)) return 0f;
        float drained = comp.DrainForConsume(amountPoolUnits, null, false);
        if (drained > 0f) SyncLactatingChargeFromPool(pawn);
        return drained;
    }
}
