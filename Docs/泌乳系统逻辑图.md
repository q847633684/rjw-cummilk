...
| **双池** | 左乳/右乳多对独立池，总满度=所有乳池之和。**结构**：GetBreastPoolEntries 按 BodyPartRecord（Part.def.defName 如 LeftBreast_1/RightBreast_1）或 defName_index 生成条目，同一对左右共享 PairIndex（0=第一对、1=第二对…）。**容量**：每乳容量 = 该乳 Severity×容量倍率（左对左、右对右）；总容量=所有乳容量之和。**流速**：每乳流速 = 该乳 RJW 流速倍率 × L×饥饿×Conditions×Genes×流速倍率；双池总流速=所有乳流速之和（不按容量比例重分配）。**进水**：每 30 tick 按对分组，每对用 LactationPoolState.TickGrowth 进水；**撑大按对**：仅当该对两侧都达基础容量后，该对才允许向 1.2× 撑大，否则只填到基础容量。**挤奶**：从第一对开始，比较该对左/右水位，选最满一侧排空并产出奶。**吸奶**：GreedyConsume 内 DrainForConsume 从第一对开始、选该对最满一侧扣量，不够再扣同对另一侧、再下一对，直至扣够或池空。未启用 RJW 乳房尺寸时视为一对，左右各 0.5。 |
...
- **挤奶（Gathered）**：取 PairIndex=0 的第一对，比较该对左/右池水位，选较满一侧（相同时**男左女右**），排空该侧并产出奶；仅排第一对的一侧，不跨对。
...
| **健康页每个乳房 hediff 悬停** | 该乳所在**该对**的「左乳：奶量/容量，右乳：奶量/容量」 | Milking.Hediff_TipString_BreastPool_Patch（Hediff.get_TipString Postfix）；通过 GetPoolEntryForBreastHediff / GetPoolEntriesForBreastHediff 取该对左右池 |
