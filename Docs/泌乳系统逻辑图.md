...
| **双池** | 左乳/右乳多对独立池，总满度=所有乳池之和。**结构**：GetBreastPoolEntries 按 BodyPartRecord（Part.def.defName 如 LeftBreast_1/RightBreast_1）或 defName_index 生成条目，同一对左右共享 PairIndex（0=第一对、1=第二对…）。**容量**：每乳容量 = 该乳 Severity×容量倍率（左对左、右对右）；总容量=所有乳容量之和。**流速**：每乳流速 = 该乳 RJW 流速倍率 × L×饥饿×Conditions×Genes×流速倍率；双池总流速=所有乳流速之和（不按容量比例重分配）。**进水**：每 30 tick 按对分组，每对用 LactationPoolState.TickGrowth 进水；**撑大按对**：仅当该对两侧都达基础容量后，该对才允许向 1.2× 撑大，否则只填到基础容量。**挤奶/吸奶**：与吸奶共用 DrainForConsume 逻辑，从第一对开始按对顺序、每对选较满一侧（相同时男左女右）扣量，直至取满当次需求量或池空；挤奶再按实际扣量产出奶。未启用 RJW 乳房尺寸时视为一对，左右各 0.5。 |
...
flowchart TB
    ...
    subgraph 进水["进水阶段"]
        ...
        E --> F[按对分组 PairIndex]
        F --> G[每乳流速 = 该乳RJW × L×饥饿×Conditions×Genes×流速倍率]
        G --> H[每对 TickGrowth 进水；撑大仅当该对两侧都达基础容量]
        H --> L[各左池水位↑]
        H --> R[各右池水位↑]
    end

    subgraph 容量["容量（每对左对左、右对右）"]
        I[每乳容量 = 该乳 Severity×容量倍率]
        J[总容量 = 所有乳容量之和]
        I --> J
    end
```
...
- **双池流速**：不再先算总流速。**左池流速** = 左乳 RJW fluidMultiplier × L × 饥饿系数 × Conditions × Genes × **流速倍率**（defaultFlowMultiplierForHumanlike，默认 2）；**右池流速** = 右乳 RJW × 同上。单次剂量、容量 2 时约 1 游戏日灌满。**左/右乳流速倍率**：仅取自对应 hediff（Breast_Left/Breast_Right）的 fluidMultiplier，Clamp 0.1～3；无该侧 hediff 或非 SexPart 时该侧=**0**（与容量一致）；未启用 RJW 乳房尺寸时左右均为 1。单乳时右乳无 hediff 故右=0，仅左池进水。
- **双池容量**：左对左、右对右。**优先**使用独立左右乳（GetLeftBreast/GetRightBreast，defName 为 Breast_Left/Breast_Right）；若无左乳且仅有旧 RJW 一对 Breasts（1 个 hediff），则兼容拆分为 Breast_Left + Breast_Right（各一半 Severity）再取。否则回退：左池容量 = 左乳 hediff Severity×系数（Clamp 0.01～10），右池容量 = 右乳 hediff Severity×系数；**RJW 一对乳房只传 1 个 hediff 时**（未拆分或非人形）：人形左右池各取该 hediff Severity 的一半（0.5+0.5），动物单乳仅左池。**无 RJW / 未开启「RJW 乳房」/ 列表为空时使用人类默认容量**（单侧 0.5、总 1）。总容量 = 左+右。
  - **Conditions**：乳腺炎等健康惩罚，无问题=1，最低 0.5。
  - **Genes**：rjw-genes 基因加成（如大胸/多乳），无=1，Clamp 0.5～1.5。
  - **RJW**：左乳用左乳 HediffDef_SexPart.fluidMultiplier，右乳用右乳的；各 Clamp 0.1～3。无该侧 hediff 或非 SexPart 则该侧流速倍率=**0**。未启用 rjwBreastSizeEnabled 时左右流速倍率均为 1。rjw-genes 未改乳房 fluidMultiplier，与 Genes 倍率无重复。
